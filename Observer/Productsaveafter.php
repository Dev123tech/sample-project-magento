...
{   
    ...
    public function sendProductToApi($_product){
        ...
        $customColor = $_product->getData('custom_color');
    
        $collection = $this->_collectionFactory->create();
        if($collection->count() > 0){
            foreach ($collection as $item) {
                $productAttrM = $item['magento_product_attribute'];
                $googleAttr = $item['google_product_attribute'];

                $data = new CustomAttribute();
                $data->setName($googleAttr);
                $data->setValue($_product->getData($productAttrM));
